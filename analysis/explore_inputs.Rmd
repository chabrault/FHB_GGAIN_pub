...
length(intersect(kasp.gid, input$INFO$GID)) # 1543
```
...
For FHB data (URN FHB and URSN), calculate the number of common genotypes between years.

Format inputs

```{r}
head(dat.fhb)
table(dat.fhb$NURSERY)

genoYr.URNFHB <- purrr::map_dfr(unique(dat.urn.fhb$YEAR), \(yr) {
  dat.yr <- dat.urn.fhb |> filter(YEAR == yr) 
  data.frame(YEAR = yr, uniq_geno = unique(dat.yr$GID))
})

genoYr.URSN <- purrr::map_dfr(unique(dat.ursn$YEAR), \(yr) {
  dat.yr <- dat.ursn |> filter(YEAR == yr) 
  data.frame(YEAR = yr, uniq_geno = unique(dat.yr$GID))
})
```


Calculate the number of common genotypes for each nursery
```{r}
cmon.genoYr.URNFHB <- expand.grid(YEAR1 = unique(dat.urn.fhb$YEAR), 
                                  YEAR2 = unique(dat.urn.fhb$YEAR)) |> 
  rowwise() |> 
  mutate(n_common_geno = length(intersect(genoYr.URNFHB$uniq_geno[genoYr.URNFHB$YEAR == YEAR1],
                                          genoYr.URNFHB$uniq_geno[genoYr.URNFHB$YEAR == YEAR2])))
summary(cmon.genoYr.URNFHB$n_common_geno)

cmon.genoYr.URSN <- expand.grid(YEAR1 = unique(dat.ursn$YEAR),
                                YEAR2 = unique(dat.ursn$YEAR)) |>
  rowwise() |>
  mutate(n_common_geno = length(intersect(genoYr.URSN$uniq_geno[genoYr.URSN$YEAR == YEAR1],
                                          genoYr.URSN$uniq_geno[genoYr.URSN$YEAR == YEAR2])))
summary(cmon.genoYr.URSN$n_common_geno)
```

```{r}
dat2plot <- bind_rows(
  cmon.genoYr.URNFHB |> mutate(NURSERY="URN FHB"),
  cmon.genoYr.URSN |> mutate(NURSERY="URSN")
)
ggplot(dat2plot, aes(x=YEAR1, y=YEAR2, fill=n_common_geno))+
  geom_tile(color="white", alpha=0.6)+
  geom_text(aes(label=n_common_geno), size=2.5)+
  paletteer::scale_fill_paletteer_c("grDevices::ag_Sunset") +
  theme_bigstatsr(size.rel=0.8)+
  scale_x_continuous(breaks=seq(1995,2024,2))+
  scale_y_continuous(breaks=seq(1995,2024,2))+
  labs(x="Year", y="Year", fill="Number of common genotypes")+
  guides(fill=guide_colorbar(barwidth=15,label.position="bottom"))+
  ...
  facet_wrap(~NURSERY)
ggsave(here("output","figures","URN_URN-FHB_URSN_common_genotypes_year_heatmap.png"), width=9, height=7)
```
...
length(unique(dat.urn.fhb$ENV))

## top ten most tested genotypes in URN FHB (213 envs)
dat.urn.fhb |> group_by(GID) |> summarize(n=n()) |> 
  arrange(desc(n)) |> head(n=10)
## top ten most tested genotypes in URSN (153 envs)
dat.ursn |> group_by(GID) |> summarize(n=n()) |> 
  arrange(desc(n)) |> head(n=10)
