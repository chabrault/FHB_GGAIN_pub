...
dat.exp <- dat |> select(GID, LOC, YEAR, NURSERY) 
...
geno.compo <- rbind(data.frame(GID=gid.ursn, NURSERY="URSN"),
                    data.frame(GID=gid.urn.fhb, NURSERY="URN"))
geno.compo <- merge(geno.compo, dat.info[,c("GID","SOURCE","FIRST_YEAR")], by="GID", all.x=TRUE)
geno.compo |> group_by(SOURCE,NURSERY,FIRST_YEAR) |> 
  ...
  #paletteer::scale_fill_paletteer_d("colorblindr::OkabeIto_black", name="Nursery") +
  theme(panel.grid = element_blank(),legend.position = "bottom")
ggsave(here("output","figures","URSN_URN_FHB_genotype_composition_source_1995-2024.png"),
