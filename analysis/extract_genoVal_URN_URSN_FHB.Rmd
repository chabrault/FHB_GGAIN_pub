...
summ.traits$nobs <- apply(dat[,traits],2,function(x) sum(!is.na(x)))
print_table(as.data.frame(cbind(traits,summ.traits)))
```

DON key numbers
```{r}
dat |> filter(!is.na(DON)) |> 
  droplevels() |> 
  summarise(n=n(), #min=min(DON), max=max(DON), mean=mean(DON), median=median(DON), sd=sd(DON),
            nENvs=length(unique(ENV)),
            meanGenoByEnv=mean(tapply(GID, ENV, function(x) length(unique(x)))),
            meanGenoByYear=mean(tapply(GID, YEAR, function(x) length(unique(x)))),
            nGeno=length(unique(GID)))
dat.noNADON <- dat |> filter(!is.na(DON)) |> droplevels()
tapply(dat.noNADON$GID, dat.noNADON$YEAR, function(x) length(unique(x)))
```
