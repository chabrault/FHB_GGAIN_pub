---
title: "Extract genotypic values for URN and URSN FHB phenotypic data"
author: Charlotte Brault, adapted from Timothée Flutre
...
```{r current dir}
here::i_am("analysis/extract_genoVal_URN_URSN_FHB.Rmd")
...
p2f <- here("data","dat_FHB_URSN_URNFHB.tsv")
file.mtime(p2f)
...
dat <- dat %>% mutate_at(all_of(info.cols), as.factor)
...
transf <- c(DIS="sqrt", VSK="sqrt",SEV="sqrt", INC="squared",DON="log10",HD="none")
...
for(tr in traits){
  write(paste0(tr,"\n"),stderr())
  dat.noNA[[tr]] <- na.omit(droplevels(dat[,c(tr, "GID","NURSERY","YEAR","LOC")]))
...
## Extract non-genetic trend 

From year effect
```{r year_effect}
yearEff <- list()
for(tr in traits){
  yearEff[[tr]] <- fixef(out[[tr]]$bestmod.reml)[grep("YEAR",names(fixef(out[[tr]]$bestmod.reml)))]
  names(yearEff[[tr]]) <- gsub("YEAR","",names(yearEff[[tr]]))
}
saveRDS(yearEff, file=here("output","lme4",paste0(par,"_year_effects_fixed_lme4.rds")))
```
...
<!-- ```{r bestmod_confint_prof} -->

<!-- for(tr in traits){ -->
<!--   p2f <- here("output","lme4",paste0("URSN_lme4_save_fit_confint-prof_",tr,par, ".RData")) -->

<!--   if(! file.exists(p2f)){ -->
<!--     st <- system.time( -->
<!--       prof <- profile(out[[tr]]$bestmod.ml, signames=FALSE, -->
...
<!--   ## dont't plot, too many factor levels -->

<!--   # idx.varcomps <- grep("\\||sigma", rownames(ci)) -->
<!--   # idx.fix <- setdiff(1:nrow(ci), idx.varcomps) -->
<!--   # plot(xyplot(prof, absVal=TRUE, conf=c(0.8, 0.95), which=idx.varcomps,main=paste0(tr))) -->
<!--   # print(xyplot(prof, absVal=TRUE, conf=c(0.8, 0.95), which=idx.fix,main=paste0(tr))) -->
<!--   # print(densityplot(prof,main=paste0(tr))) -->
<!--   # print(splom(prof, conf=c(0.8, 0.95), which=idx.varcomps,main=paste0(tr))) -->
<!--   # if(length(idx.fix) > 1) -->
<!--   #   print(splom(prof, conf=c(0.8, 0.95), which=idx.fix,main=paste0(tr))) -->
<!-- } -->
<!-- ``` -->
...
<!-- ```{r bestmod_confint_boot} -->
<!-- boot_list <- list() -->

<!-- for(tr in traits){ -->
<!--   print(tr) -->
<!--   p2f <- here("output","lme4",paste0("URSN_lme4_save_fit_confint-boot_URSN_",tr,par, ".RData")) -->

<!--   if(! file.exists(p2f)){ -->
<!--     st <- system.time( -->
<!--       fit.boot <- bootMer(x=out[[tr]]$bestmod.ml,  -->
<!--                           FUN=H2_list[[tr]]$sryStat, -->
<!--                           nsim=1*10^3,  -->
<!--                           seed=1859, -->
...
info2save <- data.frame(trait=traits,
                        ...
                        # var.geno=round(unlist(lapply(vc.reml, function(x)x[x$grp == "GID","vcov"])),3),
                        # var.resid=round(unlist(lapply(vc.reml, function(x)x[x$grp == "Residual","vcov"])),3),
                        # var.tot=round(unlist(lapply(vc.reml, function(x) sum(x$vcov))),3),
                        reliability=rel)

print_table(info2save)
...
## de transform data
geno_blues$INC$geno.blues <- sqrt(geno_blues$INC$geno.blues)
geno_blues$SEV$geno.blues <- (geno_blues$SEV$geno.blues)^2
geno_blues$DIS$geno.blues <- (geno_blues$DIS$geno.blues)^2
geno_blues$VSK$geno.blues <- (geno_blues$VSK$geno.blues)^2
geno_blues$DON$geno.blues <- 10^(geno_blues$DON$geno.blues)
...
ap(geno_blues)
"00EPWB-27" %in% geno_blues$DIS$GID
```
