...
gv.qtl$SOURCE[gv.qtl$SOURCE == "OTH"] <- "OTHER"
```
...
## Fhb1 effect, first year interaction
...
for(tr in FHB.traits){
  dat.fit <- gv.qtl |>
    dplyr::select(all_of(c("GID","FIRST_YEAR","SOURCE", "QTL.name", "allele", tr))) |>
    filter(!is.na(.data[[tr]]),FIRST_YEAR >= 1995,
           QTL.name %in% "Fhb1",
           allele %in% c("Res","Sus")) |> 
    mutate(allele=as.factor(allele))
  
  fit[[tr]] <- lm(paste0(tr, "~ FIRST_YEAR + allele + allele:FIRST_YEAR"), data=dat.fit)
  #print(anova(fit[[tr]]))
  broom::glance(fit[[tr]])
  print(summary(fit[[tr]]))
}
```
TODO: fit a mixed model with the GRM matrix to see if the pathogen is evolving or if it is due to polygenic effect?

```{r}
for(tr in FHB.traits){
  
  data = broom::augment(fit[[tr]])
  ## plot initial data and fitted line
  plot(ggplot(data, aes(x=FIRST_YEAR, y=.data[[tr]], color=allele))+
         geom_point()+
         geom_line(data=data, aes(x=FIRST_YEAR, y=.fitted, group=allele), size=1)+
         labs(x="First year", y=tr)+
         theme_bigstatsr())
}
```
...
```{r study_fhb1_effect_6traits_v2, fig.height=8}
...
for(tr in traits.sel){
  dat2plot <- gv.qtl |>
    ...
    mutate(label=factor(ifelse(allele %in% "Resistant","*Fhb1*+","*Fhb1*-"),c("*Fhb1*+","*Fhb1*-")),
           SOURCE=as.factor(SOURCE))
  table(dat2plot$SOURCE)
  ## Estimate the effect of Fhb1 on the trait while controlling for the first year and source
  #fit <- lm(paste0(tr, "~  FIRST_YEAR + allele + SOURCE"), data=dat2plot)
  fit <- lme4::lmer(paste0(tr, "~  FIRST_YEAR + allele + (1|SOURCE)"), data=dat2plot)
  source_icc <- performance::icc(fit)$ICC_conditional
  print(car::Anova(fit,test.statistic="F", type = "II"))
  #pval <- anova(fit)[["Pr(>F)"]][2]
  pval <- car::Anova(fit,test.statistic="F", type = "II")[["Pr(>F)"]][2]
  emm <- as.data.frame(emmeans::emmeans(fit, specs = "allele"))
  marginal.means[[tr]] <- emm
  #eta2 <- effectsize::eta_squared(fit)
  om2 <- effectsize::omega_squared(fit, partial=TRUE)
  
  ## add number of obs
  xlabs <- paste(levels(dat2plot$label),
                 "<br>(N=",table(dat2plot$allele),")",sep="")
  ...
  plist[[tr]] <- ggplot(dat2plot, aes(x=allele, y=.data[[tr]]))+
    ...
    annotate("text",label=paste0("\u03c9\u00b2 = ",round(om2$Omega2_partial[2],4)),
...
plist2 <- list(DIS=plist$DIS,YLD=plist$YLD,VSK=plist$VSK,
               PROT=plist$PROT,DON=plist$DON,TWT=plist$TWT)
ggarrange(plotlist=plist2, ncol=2, nrow=3,common.legend = TRUE, legend="bottom",
          labels = c("A","D","B","E","C","F"))
ggsave(here("output","figures",paste0(pop_code,"_Fhb1_effects_6traits_distrib_vRev.png")),
       height=8,width=11,scale=0.85)
# ## Export for FHB forum
# ggarrange(plotlist=plist2, ncol=2, nrow=3,
#           common.legend = TRUE, legend="bottom",labels = NULL)
# ggsave(here("output","figures",paste0(pop_code,"_Fhb1_effects_6traits_distrib_v5.png")),
#        height=8,width=11,scale=0.85)
```
...
tmp <- expand.grid(trait=c("DIS","VSK","DON"),
                   allele=c("Fhb1+","Fhb1-"))
tmp$y <- c(44,47,31,40,44,27)
ggain<- ggain |> 
  mutate(leg=paste0(allele," slope: ", slope, " [",slope.min,",",slope.max,"]")) |> 
  left_join(tmp, by=c("trait","allele"))
...
ggplot(dat2plot,aes(x=FIRST_YEAR, y=meanYr, color=allele))+
  ...
  geom_label(data=ggain, mapping=aes(x=2015, y=y, label=leg, color=allele),
            size=4.2, show.legend=F,alpha=0.7, fill="white") +
  theme_bigstatsr(size.rel = 0.8)+
...
for(tr in traits.sel){
  dat2plot <- gv.qtl |>
    ...
    mutate(allele=factor(allele, levels=c("Res","Sus"),
                         labels=c("Resistant","Susceptible")))|> 
    mutate(label=factor(ifelse(allele %in% "Resistant","*Fhb5*+","*Fhb5*-"),c("*Fhb5*+","*Fhb5*-")),
           SOURCE=as.factor(SOURCE))
  ## Estimate the effect of Fhb1 on the trait while controlling for the first year and source
  #fit <- lm(paste0(tr, "~  FIRST_YEAR + allele + SOURCE"), data=dat2plot)
  fit <- lme4::lmer(paste0(tr, "~  FIRST_YEAR + allele + (1|SOURCE)"), data=dat2plot)
...
ggarrange(plotlist=plist2, ncol=2, nrow=3,common.legend = TRUE, legend="bottom",
          labels = c("A","D","B","E","C","F"))
ggsave(here("output","figures",paste0(pop_code,"_Fhb5_effects_6traits_distrib_vRev.png")),
       height=8,width=11,scale=0.85)
```
...
for(tr in traits.sel){
  dat2plot <- gv.qtl |>
    ...
    mutate(allele=factor(allele, levels=c("Res","Sus"),
                         labels=c("Resistant","Susceptible")))|> 
    mutate(label=factor(ifelse(allele %in% "Resistant","*Fhb5AS*+","*Fhb5AS*-"),
                        c("*Fhb5AS*+","*Fhb5AS*-")),
           SOURCE=as.factor(SOURCE))
...
ggarrange(plotlist=plist2, ncol=2, nrow=3,common.legend = TRUE, legend="bottom",
          labels = c("A","D","B","E","C","F"))
ggsave(here("output","figures",paste0(pop_code,"_Fhb5AS_effects_6traits_distrib_vRev.png")),
       height=8,width=11,scale=0.85)
```
...
for(tr in traits.sel){
  ...
  ## Estimate the effect of Fhb1 on the trait while controlling for the first year and source
  fit <- lm(paste0(tr, "~ 1+ FIRST_YEAR + SOURCE + Fhb5AS * Fhb1 * Fhb5"), data=dat2fit)
  fit.step <- step(fit)
  #anova(fit.step)
...
Add this table for all FHB traits?
...
nrow(df.qtls |> filter(QTL.name %in% "Fhb1", allele %in% "Sus")) ## 788
