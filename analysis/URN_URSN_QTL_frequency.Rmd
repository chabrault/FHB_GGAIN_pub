...
```{r combine_kasps}
qtl.desc <- mas.tabL.FHB |> select(QTL.name, KASP.name) |> distinct()
...
ggplot(dat2plot,aes(x=FIRST_YEAR, y=freq.slid, color=SOURCE))+
  ...
  theme_bigstatsr(size.rel=0.8)+
  gghighlight::gghighlight(line_label_type="ggrepel_label", label_params =list(nudge_x=1.5, alpha=0.7, min.segment.length=0)) +
  scale_y_continuous(labels = scales::percent)+
