...
ggplot(dat2plot,aes(x=FIRST_YEAR, y=freq.slid, color=KASP.name))+
  ...
  scale_x_continuous(breaks=seq(1925,2015,10), limits=range(dat2plot$FIRST_YEAR))+
...
ggsave(here("output","figures",paste0(pop_code,"_Fhb1-Fhb5-Fhb5AS_freq_by-nursery_",
                                      "first-year_5-yr-window_v2.png")),
       height=7,width=7,scale=0.9)
```
Only for Fhb1
```{r}

ggplot(filter(dat2plot, QTL.name %in% "Fhb1"),
       aes(x=FIRST_YEAR, y=freq.slid, color=NURSERY, group=NURSERY, shape=NURSERY))+
  #facet_wrap(~QTL.name, nrow=3)+lemon::coord_capped_cart(bottom="both")+
  #lemon::facet_rep_wrap(.~QTL.name, nrow=3,repeat.tick.labels = F)+
  #ggh4x::facet_wrap2(.~QTL.name, nrow=3,axes = "all", remove_labels = "all")+
  geom_point(size=3,show.legend=T)+
  geom_line(lwd=2,alpha=0.5, show.legend=F)+
  theme_bigstatsr(size.rel=0.85)+
  scale_color_manual(values = c("URN"="#00BFFF","URSN"="#9A32CD"),name="Nursery")+
  scale_shape_manual(values=c(16,17), "Nursery") +
  scale_x_continuous(breaks=seq(1995,2024,5),minor_breaks = seq(1995,2024,1))+
  guides(color=guide_legend(title="Nursery", 
                            override.aes=list(shape=c(16,17), size=3)))+
  # gghighlight::gghighlight(line_label_type="ggrepel_label",calculate_per_facet=T,
  #                          label_params=list(nudge_x=1)) +
  scale_y_continuous(labels = scales::percent)+
  ylab("Frequency of *Fhb1* resistance allele")+
  xlab("First year of testing")+
  theme(legend.position="inside",
        ...
        panel.grid.major = element_line(colour="grey35", linewidth = 0.07),
        panel.grid.minor.x = element_line(colour="grey70", linewidth = 0.03))
ggsave(here("output","figures",paste0(pop_code,"_Fhb1_freq_by-nursery_first-year_5-yr-window.png")),
       height=5,width=7.5,scale=0.9)
```
...
mas.tabL.FHB |> dplyr::select(QTL.name, KASP.name) |> distinct() |> print_table()
## Fhb1
tmp.qtl <- mas.tabL.FHB |> filter(QTL.name == "Fhb1") |> 
  dplyr::select(-QTL.name,-QTL.allele) |> distinct() |> 
  tidyr::pivot_wider(names_from=KASP.name, values_from=allele) |>
