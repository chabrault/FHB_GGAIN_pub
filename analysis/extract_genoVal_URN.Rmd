...
for(tr in traits){
  ...
  if(length(unique(as.character(dat.noNA[[tr]]$YEAR))) < 2){
    ...
    p2f.save <- here("output","lme4",paste0("lme4-output_",tr,"-trait.rds"))
    if(!file.exists(p2f.save)){
      ...
      saveRDS(out[[tr]],p2f.save)
    } else {
      out[[tr]] <- readRDS(p2f.save)
    }
...
```{r}
for(tr in traits[!traits %in% pbm]){
  fit.all[[tr]]$YEAR <- droplevels(as.factor(fit.all[[tr]]$YEAR))
  x.lim <- max(abs(fit.all[[tr]]$scl.cond.res), na.rm=TRUE)
  g <- ggplot(fit.all[[tr]], aes(x=scl.cond.res, y=fitted,color=YEAR)) +
    ...
    geom_vline(xintercept = c(0, -1.96, 1.96),
               linetype="dashed")
  if(nlevels(fit.all[[tr]]$YEAR) < 10){
    g <- g+paletteer::scale_colour_paletteer_d("Polychrome::palette36")
  } else {
