...
## set the earliest year as 1995
dat |> filter(FIRST_YEAR < 1995) |> 
  dplyr::select(GID,SOURCE,FIRST_YEAR,NURSERY) |> distinct()
idx <- dat$FIRST_YEAR < 1995
dat$FIRST_YEAR[idx] <- 1995
...
```{r}
tr <- "DIS"
dat2plot <- dat |> dplyr::select(-INC,-SEV,-HD)|>
  tidyr::pivot_longer(cols=all_of(traits), names_to="trait", values_to="value") |> 
  mutate(ISMT=ifelse(SOURCE %in% "MT", TRUE,FALSE)) |> 
...
yearEff <- readRDS(p2f)
ap(yearEff)
## year effect not selected for DIS
yearEff$DIS <- NULL
...
ggplot(tbl.yrEff, aes(x=year, y=yearEff)) +
  ...
  geom_point( size=2.5, alpha=0.8, show.legend=F) +
  labs(x="Year", y="Estimated year effect") +
   scale_color_manual(values=c("DIS"="#F8766D","VSK"="#00BA38","DON"="#619CFF"))+
   scale_x_continuous(breaks=seq(year.range.plt[1],year.range.plt[2],5),
                      minor_breaks = seq(year.range.plt[1],year.range.plt[2],1))+
  #geom_smooth()+
  bigstatsr::theme_bigstatsr(size.rel=0.8)+
...
ggsave(here("output","figures","URSN_URN_FHB_non-genetic-year-effects.png"),
       width=7, height=5, scale=0.85)
```
...
```{r}
dat.long <- dat |> dplyr::select(-INC,-SEV,-HD) |> 
